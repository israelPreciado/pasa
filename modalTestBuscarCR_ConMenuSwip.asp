...
<thead><tr><th>&nbsp;</th><th>CR</th><th>Sucursal</th><th>Contratista</th><th>Supervisor</th><th>Fecha Inicio</th><th>Fecha Fin</th><th>Estatus</th><%if estatus1 =0 and estatus2 =0 then%><th>Obs.</th><%end if%><th>Costo Estimado</th></tr></thead>
<tbody>
<%
set conn3=server.CreateObject("adodb.connection")
set rs1=server.CreateObject("adodb.recordset")
conn3.open connstr
'sql1= "SELECT a.cr, b.NOMBRE_SUCURSAL, c.contratista,e.login_nombre, b.INICIOPROYECTO, d.EstatusObra, f.total FROM tseguimiento a LEFT JOIN cat_sucursales b ON a.cr = b.cr LEFT JOIN cat_contratistas c ON a.ContratistaAsignado = c.id_contratista LEFT JOIN cat_estatusobras d ON a.EstatusObra = d.id_estatusObra LEFT JOIN app_bnrt_login e on a.supervisorAsignado=e.login_id LEFT JOIN (select cr, round(sum(monto),2) as total from costoestimado GROUP BY cr) f on a.cr=f.cr WHERE( a.EstatusObra = '0' OR a.EstatusObra = '1' OR a.EstatusObra = '2' OR a.EstatusObra = '3' ) ;"
sql1= "SELECT a.cr, b.NOMBRE_SUCURSAL, c.contratista,e.login_nombre, b.INICIOPROYECTO,a.FechaTermino, d.EstatusObra, case when f.total is null then 0 else f.total end as total FROM tseguimiento a LEFT JOIN cat_sucursales b ON a.cr = b.cr LEFT JOIN cat_contratistas c ON a.ContratistaAsignado = c.id_contratista LEFT JOIN cat_estatusobras d ON a.EstatusObra = d.id_estatusObra LEFT JOIN app_bnrt_login e on a.supervisorAsignado=e.login_id LEFT JOIN ( SELECT cr, ROUND(sum(cant * PRECIOBRUTO), 2) AS total FROM cat_conceptossucursales_sap GROUP BY cr) f ON a.cr*1 = f.cr *1 GROUP BY cr;"
...
while not (rs1.eof)
%>
<tr><td align="center"  style="cursor:pointer"  onclick="showDetalleAvance('<%=rs1("cr")%>')"><font size="+2"><span class="icon-search" ></span></font></td><td><font color="#000000"><%=rs1("cr")%></font></td><td><font color="#000000"><%=rs1("NOMBRE_SUCURSAL")%></font></td><td><font color="#000000"><%=rs1("contratista")%></font></td><td><font color="#000000"><%=rs1("login_nombre")%></font></td><td><font color="#000000"><%=rs1("INICIOPROYECTO")%></font></td><td><font color="#000000"><%=rs1("FechaTermino")%></font></td><td><font color="#000000"><%=rs1("EstatusObra")%></font></td><%if estatus1 =0 and estatus2 =0 then%><td><font color="#000000"><%if not(isnull(rs1("INICIOPROYECTO"))) then%><%if cdate(rs1("INICIOPROYECTO"))<date() then%>DESFASADA<%end if%><%end if%><%end if%></font></td><td style='cursor:pointer' title='Ver Detalle de Estimaci&oacute;n'><font color="#000000"><%=formatoPesos(nuevoCostoEstimado(rs1("cr")))%></font></td></tr>
<%
rs1.movenext
	wend 
rs1.close
...
<%else%>
<table cellpadding="0" cellspacing="0" class="display" id="example1">
<thead><tr><th>&nbsp;</th><th>CRs</th><th>Sucursal</th><th>Contratista</th><th>Supervisor</th><th>Fecha Inicio</th><th>Fecha Fin</th><th>Estatus</th><%if estatus1 =0 and estatus2 =0 then%><th>Obs.</th><%end if%><th>Costo Estimado</th></tr></thead>
<tbody>
<%
set conn3=server.CreateObject("adodb.connection")
set rs1=server.CreateObject("adodb.recordset")
