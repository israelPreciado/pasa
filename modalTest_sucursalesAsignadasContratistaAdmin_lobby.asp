...
<thead>
	<tr>
		<th width="10%">&nbsp;</th><th width="10%">CR</th><th width="30%">Sucursal</th><th width="20%">Territorial</th><th width="10%">Zona</th><th width="10%">Contratista</th><th width="10%">Supervisor</th><th width="10%">Costo Estimado</th>
	</tr>
</thead>
<tbody>
<%
...
'sql1="select cr,NOMBRE_SUCURSAL,Territorial,ZONA, case when costo is null then 0 else costo end as costo,case when estatus >0 then 1 else 0 end as estatus from (select a.cr,b.NOMBRE_SUCURSAL,b.Territorial,b.ZONA,c.costo,c.estatus from tseguimiento a LEFT JOIN cat_sucursales b on a.cr=b.cr LEFT JOIN (select cr, case when costo is null then 0 else costo end as costo,case when estatus is null then 0 else estatus end as estatus from (select cr, sum(round(cantidad * pu,2)) as Costo,case when estatus is null then 0 else estatus end as estatus  from nuevocosto_estimadoobra  GROUP BY cr) a)c on a.cr=c.cr where ContratistaAsignado='" & session.contents("contratista") & "') a"
 sql1="SELECT cr, nombre_lobby, Territorial, ZONA, CASE WHEN costo IS NULL THEN 0 ELSE costo END AS costo, CASE WHEN estatus > 0 THEN 1 ELSE 0 END AS estatus, CASE WHEN contratista IS NULL THEN 'No asignado' ELSE contratista END AS contratista, CASE WHEN supervisor IS NULL THEN 'No asignado' ELSE supervisor END AS supervisor FROM( SELECT a.cr, a.nombre_lobby, d.territorial, a.id_zona AS zona, c.costo, c.estatus, e.contratista, f.login_nombre AS supervisor FROM tseguimiento_lobby a LEFT JOIN cat_sucursales b ON a.cr = b.cr LEFT JOIN cat_contratistas e ON a.id_proveedor = e.id_contratista LEFT JOIN app_bnrt_login f ON a.id_supervisor = f.login_id LEFT JOIN cat_territoriales d ON a.id_territorio = d.id_territorial LEFT JOIN( SELECT cr, CASE WHEN costo IS NULL THEN 0 ELSE costo END AS costo, CASE WHEN estatus IS NULL THEN 0 ELSE estatus END AS estatus FROM( SELECT cr, sum(round(cantidad * pu, 2)) AS Costo, sum(estatus) AS estatus FROM nuevocosto_estimadoobra_lobby GROUP BY cr) a) c ON a.cr = c.cr) a"
rs1.open sql1, conn3
if rs1.eof then
	rs1.close
	conn3.close
...
<tr>
 ...
 <td align="center" style="cursor:pointer"  ><font color="#000000"><%=rs1("Contratista")%></font></td>
 <td align="center" style="cursor:pointer"  ><font color="#000000"><%=rs1("supervisor")%></font></td>
