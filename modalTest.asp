...
<thead><tr><th>&nbsp;</th><th>CR</th><th>Suc.</th><th>Superv.</th><th>Contrat.</th><th>Fecha Inicio</th><th>Fecha Fin</th><th>Fecha Reprog.</th><th>% Avance</th><th><%if estatus1 =3 and estatus2 =3 then%>Pend.<%else%>Estatus<%end if%></th></tr></thead>
<tbody>
<%
set conn3=server.CreateObject("adodb.connection")
set rs1=server.CreateObject("adodb.recordset")
conn3.open connstr
sql1= "select a.cr,b.NOMBRE_SUCURSAL, b.Territorial,e.login_nombre as supervisor,d.contratista,b.INICIOPROYECTO,b.FINPROYECTO, case when c.pendientes is null then 0 else c.pendientes end as pendientes, a.fechareasignacion, a.avancereal from tseguimiento a LEFT JOIN cat_sucursales b on a.cr=b.CR LEFT JOIN (select cr,count(*) as pendientes from pendientes where id_estatus=1 GROUP BY cr) c on a.CR=c.cr LEFT JOIN cat_contratistas d on a.ContratistaAsignado=d.id_contratista left join app_bnrt_login e on a.supervisorAsignado=e.login_id where EstatusObra=3 and b.territorial='" & territorial & "';"
...
%>
<tr><td align="center" style="cursor:pointer" onclick="showDetalleAvance('<%=rs1("cr")%>')"><font size="+2"><span class="icon-search"></span></font></td><td><font color="#000000"><%=rs1("cr")%></font></td><td><font color="#000000"><%=rs1("NOMBRE_SUCURSAL")%></font></td><td><font color="#000000"><%=Server.HTMLEncode(rs1("supervisor"))%></font></td><td><font color="#000000"><%=Server.HTMLEncode(rs1("contratista"))%></font></td><td><font color="#000000"><%=rs1("inicioPROYECTO")%></font></td><td><font color="#000000"><%=rs1("FINPROYECTO")%></font></td><td><font color="#000000"><%=rs1("fechareasignacion")%></font></td><td><font color="#000000"><%=rs1("avancereal")%></font></td><td><font color="#000000"><%=rs1("pendientes")%></font></td></tr>
<%
rs1.movenext
	wend 
rs1.close
...
<%if estatus1="4" then%>
<table cellpadding="0" cellspacing="0" class="display" id="example1">
<thead><tr><th>&nbsp;</th><th>CR</th><th>Suc.</th><th>Contrat.</th><th>Fecha Inicio</th><th>Fecha Fin</th><th>Fecha Reprog.</th><th>% Avance</th><th><%if estatus1 =4 and estatus2 =4 then%>Observ.<%else%>Estatus<%end if%></th></tr></thead>
<tbody>
<%
set conn3=server.CreateObject("adodb.connection")
set rs1=server.CreateObject("adodb.recordset")
conn3.open connstr
sql1= "SELECT a.cr, b.NOMBRE_SUCURSAL, c.contratista,e.login_nombre, b.INICIOPROYECTO,a.FechaTermino, d.EstatusObra, case when f.total is null then 0 else f.total end as total,a.comentarioEstatus, a.fechareasignacion, a.avancereal FROM tseguimiento a LEFT JOIN cat_sucursales b ON a.cr = b.cr LEFT JOIN cat_contratistas c ON a.ContratistaAsignado = c.id_contratista LEFT JOIN cat_estatusobras d ON a.EstatusObra = d.id_estatusObra LEFT JOIN app_bnrt_login e on a.supervisorAsignado=e.login_id LEFT JOIN ( SELECT cr, ROUND(sum(cant * PRECIOBRUTO), 2) AS total FROM cat_conceptossucursales_sap GROUP BY cr) f ON a.cr*1 = f.cr *1 WHERE( a.EstatusObra = '"& estatus1 & "' OR a.EstatusObra = '"& estatus2 & "') AND b.territorial = '"& territorial &"' GROUP BY cr;"
...
%>
<tr><td align="center" style="cursor:pointer" onclick="showDetalleAvance('<%=rs1("cr")%>')"><font size="+2"><span class="icon-search"></span></font></td><td><font color="#000000"><%=rs1("cr")%></font></td><td><font color="#000000"><%=rs1("NOMBRE_SUCURSAL")%></font></td><td><font color="#000000"><%=Server.HTMLEncode(rs1("contratista"))%></font></td><td><font color="#000000"><%=rs1("inicioPROYECTO")%></font></td><td><font color="#000000"><%=rs1("FechaTermino")%></font></td><td><font color="#000000"><%=rs1("FechaReasignacion")%></font></td><td><font color="#000000"><%=rs1("avancereal")%></font></td><td><font color="#000000"><%=rs1("comentarioEstatus")%></font></td></tr>
<%
rs1.movenext
	wend 
rs1.close
...
<%else%>
<table cellpadding="0" cellspacing="0" class="display" id="example1">
<thead><tr><th>&nbsp;</th><th>CR</th><th>Suc.</th><th>Contrat.</th><th>Superv.</th><th>Fecha Inicio</th><th>Fecha Fin</th><th>Fecha Reprog.</th><th>Estatus</th><th>% Avance</th><%if estatus1 =0 and estatus2 =0 then%><th>Obs.</th><%end if%><th>Costo Estimado</th></tr></thead>
<tbody>
<%
set conn3=server.CreateObject("adodb.connection")
set rs1=server.CreateObject("adodb.recordset")
...
sql1= "SELECT a.cr, b.NOMBRE_SUCURSAL, c.contratista,e.login_nombre, b.INICIOPROYECTO,a.FechaTermino, d.EstatusObra, case when f.total is null then 0 else f.total end as total, a.fechareasignacion, a.avancereal FROM tseguimiento a LEFT JOIN cat_sucursales b ON a.cr = b.cr LEFT JOIN cat_contratistas c ON a.ContratistaAsignado = c.id_contratista LEFT JOIN cat_estatusobras d ON a.EstatusObra = d.id_estatusObra LEFT JOIN app_bnrt_login e on a.supervisorAsignado=e.login_id LEFT JOIN ( SELECT cr, ROUND(sum(cant * PRECIOBRUTO), 2) AS total FROM cat_conceptossucursales_sap GROUP BY cr) f ON a.cr*1 = f.cr *1 WHERE( a.EstatusObra = '"& estatus1 & "' OR a.EstatusObra = '"& estatus2 & "') AND b.territorial = '"& territorial &"' GROUP BY cr;"
...
while not (rs1.eof)
%>
<tr><td align="center" <%if estatus1<>0 and estatus2 <>0 then%> style="cursor:pointer"  onclick="showDetalleAvance('<%=rs1("cr")%>')"<%end if%>><font size="+2"><span class="icon-search" <%if estatus1 =0 and estatus2 =0 then%>style="display:none"<%end if%>></span></font></td><td><font color="#000000"><%=rs1("cr")%></font></td><td><font color="#000000"><%=rs1("NOMBRE_SUCURSAL")%></font></td><td><font color="#000000"><%=rs1("contratista")%></font></td><td><font color="#000000"><%=rs1("login_nombre")%></font></td><td><font color="#000000"><%=rs1("INICIOPROYECTO")%></font></td><td><font color="#000000"><%=rs1("FechaTermino")%></font></td><td><font color="#000000"><%=rs1("fechareasignacion")%></font></td><td><font color="#000000"><%=rs1("EstatusObra")%></font></td><td><font color="#000000"><%=rs1("avancereal")%></font></td><%if estatus1 =0 and estatus2 =0 then%><td><font color="#000000"><%if not(isnull(rs1("INICIOPROYECTO"))) then%><%if cdate(rs1("INICIOPROYECTO"))<date() then%>DESFASADA<%end if%></font></td><%end if%><%end if%><td style='cursor:pointer' title='Ver Detalle de Estimaci&oacute;n' onclick="verDetalle('<%=rs1("cr")%>')"><font color="#000000"><%=formatoPesos(nuevoCostoEstimado(rs1("cr")))%></font></td></tr>
<%
rs1.movenext
	wend 
rs1.close
