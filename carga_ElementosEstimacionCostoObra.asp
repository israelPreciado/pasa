...
sql= "select * from (select a.CLAVE_SAP_CONCURSO,a.clave_sap_zona, b.DESCRIPCION_CORTA,a.pu,a.cantidad,b.UND, a.componente, concat(left(a.fecha_ingreso,4),'-',right(left(a.fecha_ingreso,7),2),'-',right(left(a.fecha_ingreso,10),2),' ',right(a.fecha_ingreso,8))  as fecha_ingreso from nuevoCosto_estimadoObra a LEFT JOIN cat_sap_2017 b on a.CLAVE_SAP_CONCURSO=b.CLAVE_SAP_CONCURSO  where cr ='" & cr & "' AND  A.componente NOT like 'ATMS' AND A.componente NOT LIKE 'otros') as a union (select clave_sap_concurso,clave_sap_zona,descorta as descripcion_corta,pu,cantidad,und,componente,fecha_ingreso from nuevocosto_estimadoobra where cr ='" & cr & "' and   componente like 'Otros') UNION (select a.CLAVE_SAP_CONCURSO,a.clave_sap_zona, b.DESCRIPCION_CORTA,a.pu,a.cantidad,b.UND, a.componente, concat(left(a.fecha_ingreso,4),'-',right(left(a.fecha_ingreso,7),2),'-',right(left(a.fecha_ingreso,10),2),' ',right(a.fecha_ingreso,8))  as fecha_ingreso from nuevoCosto_estimadoObra a LEFT JOIN cat_sap_atms_2017 b on a.CLAVE_SAP_CONCURSO=b.CLAVE_SAP_CONCURSO  where cr ='" & cr & "' AND  A.componente like 'ATMS') order by fecha_ingreso desc limit 5;"
rs25.open sql,conn5
if rs25.eof then
