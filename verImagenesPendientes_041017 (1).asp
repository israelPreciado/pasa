...
while not(rs.eof)
%>
  <img class='mySlides w3-animate-zoom' src='<%=rs("path") & rs("name")%>' style='width:100%' name='<font size="+4"><strong>Capitulo:</strong> <%=rs("modulo") & "<br/> <strong>Pendiente:</strong> " & rs("pendiente") & "<br/> <strong>Registrada Por:</strong> " & rs("id_usuario") &  "<br/> <strong>Fecha Registro:</strong> " & rs("fechaf") & "<br/> <strong>Perfil: </strong>" & rs("perfil") & "</font>"%>' id='<%=rs("id_foto")%>' onclick="rotarImagen(this.id,document.getElementById('angulo').value)">
<%
cont=cont+1
rs.movenext
wend
