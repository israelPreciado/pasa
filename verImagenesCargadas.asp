...
Conn.Open connstr	
sql="select z.*,x.zona,y.login_nombre,w.PERFIL as privilegio,t.NOMBRE_SUCURSAL,t.Territorial,p.contratista from (select *, YEAR(fecha) as anio,MONTH(fecha) as mes,day(fecha) as dia from ctrl_fotografias)z LEFT JOIN cat_zonafotografica x on z.id_zona=x.id_zona LEFT JOIN app_bnrt_login y on z.usuario=y.login_id LEFT JOIN cat_perfil w on z.perfil=w.ID_PERFIL LEFT JOIN cat_sucursales t on z.cr=t.CR LEFT JOIN cat_contratistas p on z.usuario=p.id_contratista where z.cr='"& cr &"' and day(fecha)='"& dia &"' and MONTH(fecha)='"& mes &"' and YEAR(fecha)='"& anio &"'"
rs.open sql,conn
if rs.eof then
%>
...
%>
 <img class='mySlides w3-animate-zoom' src='<%=rs("path") & rs("name")%>' style='width:100%' name='<font size="+4"><strong>Zona:</strong> <%=rs("zona") & "<br/> <strong>Comentario:</strong> " & rs("comentario") & "<br/> <strong>Registrada Por:</strong> " & rs("contratista") &  "<br/> <strong>Fecha Registro:</strong> " & rs("fecha") & "<br/> <strong>Perfil: </strong>" & rs("privilegio") & "</font> <br/><br/><br/><br/><br/>"%>' id='<%=rs("id_foto")%>' onclick="rotarImagen(this.id,document.getElementById('angulo').value)">
<%
end if
cont=cont+1
rs.movenext
