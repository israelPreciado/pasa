...
sql="update nuevoCosto_estimadoObra set avance='" & cantidad & "' where id='" & sap & "' and cr='" & cr & "'"
...
if err<>0 then
	...
	sql2="insert into nuevoCosto_estimadoObra_log (cr,componente,clave_sap_concurso,cantidad) values ('" & cr & "','UPDATE_AVANCE','" & sap & "','" & cantidad &"')"
'ejecutamos la consulta
	conn.execute sql2
				...
				while not(rs12.eof)
					if (cDbl(calculaAvanceProyectado7030(rs12("cr"),rs12("fechaInicio")))>cDbl(calculaAvanceGeneral(rs12("cr"),"G"))) then
						...
						if (cDbl(calculaAvanceProyectado7030(rs12("cr"),rs12("fechaInicio")))<cDbl(calculaAvanceGeneral(rs12("cr"),"G"))) then
							...
							sql13="update tseguimiento set EstatusObra=3 , fUltimoReporte ='" & LPad(Day(Now),2,"0") & "/" & LPad(Month(Now),2,"0") & "/" &   Year(Now) & "' where cr='" & rs12("cr") & "'"
							conn.execute sql13
							sql13="update tseguimiento set avanceProyectado=" & cDbl(calculaAvanceProyectado7030(rs12("cr"),rs12("fechaInicio"))) & " where cr='" & rs12("cr") & "'"
							conn.execute sql13
						end if			
			...
			if not(rs12.eof) then
				if (cDbl(calculaAvanceProyectado7030(cr,rs12("fechaInicio")))>cDbl(calculaAvanceGeneral(cr,"G"))) then
					...
					if (cDbl(calculaAvanceProyectado7030(cr,rs12("fechaInicio")))<cDbl(calculaAvanceGeneral(cr,"G"))) then
						...
						sql13="update tseguimiento set EstatusObra=3 , fUltimoReporte ='" & LPad(Day(Now),2,"0") & "/" & LPad(Month(Now),2,"0") & "/" &   Year(Now) & "' where cr='" & cr & "'"
						conn.execute sql13
					end if			
				end if
			end if
