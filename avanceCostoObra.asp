...
function verDetalle(componente){
	...
	document.getElementById('frm_avanceObra').action="avanceCostoObra_paso2.asp"
	document.getElementById('frm_avanceObra').submit()
}
		...
		<br />
  		<div id="logo">  
    		<p><strong>Distribuci&oacute;n del Costo de Obra</strong></p>
             <table width="100%"><tr><td width="50%"><p>Importe Contrato: <strong><%=costoEstimado(session.Contents("cr"))%></strong></p></td></tr></table> 
			 <table width="100%"><tr><td width="50%"><p>Porcentaje Avance (O.C.): <strong><%=porcentajeAvanceSucursal(session.Contents("cr"))%> %</strong></p></td></tr></table>
			 <table width="100%"><tr><td width="50%"><p>Porcentaje Avance Pesos (O.C.): <strong><%=porcentajeAvanceSucursalPesos(session.Contents("cr"))%></strong></p></td></tr></table>			 
			 <table width="100%"><tr><td width="50%"><p>Porcentaje Exceso (O.C.): <strong><%=porcentajeExceso(session.Contents("cr"))%> %</strong></p></td></tr></table>
			 <table width="100%"><tr><td width="50%"><p>Porcentaje Exceso Pesos (O.C.): <strong><%=costoExceso(session.Contents("cr"))%></strong></p></td></tr></table>
...
<table width="90%" align="center" style="padding-left:5%">
	<tr>
		<td colspan="2" align="center"><p style="font-size:38px"><strong>PARTIDA</strong></p></td>
		<td align="left"><p style="font-size:38px"><strong>IMPORTE</strong></p></td>
		<td align="left"><p style="font-size:38px"><strong>AVANCE</strong></p></td>
	</tr>
 <%
...
conn3.open connstr
if session.contents("privilegio")="2" then
sql4= "select cr,componente,sum(round(ifnull(avance,0) * pu,2)) as avance,Sum(round(ifnull(cantidad,0) * pu,2)) as estimado,estatus from nuevocosto_estimadoobra where cr ='" & session.Contents("cr") &"' GROUP BY cr,componente"
...
%>
        <tr>
		 ...
		 <td width="10%"><p style="font-size:48px"><strong><span id="search_<%=rs3("componente")%>"  onclick="verDetalle('<%=rs3("componente")%>')"class="icon-search"></span></strong></p></td>
         <td width="10%"><p style="font-size:38px"><strong><%=rs3("componente")%></strong></p></td>
         <td width="10%"><p style="font-size:38px"><strong><%=formatoPesos(rs3("estimado"))%></strong></p></td>
         <td width="15%"><p style="font-size:38px;"><strong><%=porcentajeAvancePartida(session.Contents("cr"),rs3("componente"))%> %</strong></p></td>
		<%if session.Contents("privilegio")=5 then%>
		<td width="20%"  align="right"><p style="font-size:38px"><strong><font color="red"><span onclick="cerrar('<%=rs3("componente")%>','<%=rs3("cr")%>',1)" id="unlock_<%=rs3("componente")%>" <%if rs3("estatus")="1" then%>style="display:none"<%end if%> class="icon-lock"></span></font><font color="green"><span onclick="abrir('<%=rs3("componente")%>','<%=rs3("cr")%>',0)" id="lock_<%=rs3("componente")%>" <%if rs3("estatus")="0" then%>style="display:none"<%end if%> class="icon-unlocked"></span></font></strong></p></td>
		<%end if%>
         </tr>
