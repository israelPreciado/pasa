...
function verDetalle(componente){
	...
	document.getElementById('frm_avanceObra').action="autorizarCostoObra_paso2.asp"
	document.getElementById('frm_avanceObra').submit()
}
		...
		<br />
  		<div id="logo">  
    		<p><strong>Distribuci&oacute;n del Costo de Obra</strong></p>
             <table width="100%"><tr><td width="50%"><p>Costo Estimado: <strong><%=costoEstimado(session.Contents("cr"))%></strong></p></td></tr></table>    
    	</div>
  		<div id="barrasuperior">
...
conn3.open connstr
sql4= "select cr,componente,sum(round(cantidad * pu,2)) as estimado,estatus from nuevocosto_estimadoobra where cr ='" & session.Contents("cr") &"' GROUP BY cr,componente"
...
%>
        <tr>
		 ...
		 <td width="10%"><p style="font-size:48px"><strong><span id="search_<%=rs3("componente")%>" <%if rs3("estatus")="0" then%>style="display:none"<%end if%> onclick="verDetalle('<%=rs3("componente")%>')"class="icon-search"></span><span id="nosearch_<%=rs3("componente")%>" <%if rs3("estatus")="1" then%>style="display:none"<%end if%> onclick="simulateClick(document.getElementById('alertCerrado'))" class="icon-search"></span></strong></p></td>
         <td width="10%"><p style="font-size:48px"><strong><%=rs3("componente")%></strong></p></td>
         <td width="10%"><p style="font-size:48px"><strong><%=formatoPesos(rs3("estimado"))%></strong></p></td>
		<%if session.Contents("privilegio")=5 then%>
		<td width="20%"  align="right"><p style="font-size:48px"><strong><font color="red"><span onclick="cerrar('<%=rs3("componente")%>','<%=rs3("cr")%>',1)" id="unlock_<%=rs3("componente")%>" <%if rs3("estatus")="1" then%>style="display:none"<%end if%> class="icon-lock"></span></font><font color="green"><span onclick="abrir('<%=rs3("componente")%>','<%=rs3("cr")%>',0)" id="lock_<%=rs3("componente")%>" <%if rs3("estatus")="0" then%>style="display:none"<%end if%> class="icon-unlocked"></span></font></strong></p></td>
		<%end if%>
         </tr>
