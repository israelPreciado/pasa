...
conn3.open connstr
if session.contents("privilegio")="2" then
sql4= "select cr,componente,sum(round(ifnull(avance,0) * pu,2)) as avance,Sum(round(ifnull(cantidad,0) * pu,2)) as estimado,estatus from nuevocosto_estimadoobra_lobby where cr ='" & session.Contents("cr") &"' GROUP BY cr,componente"
...
%>
        <tr>
         ...
         <td width="15%"><p style="font-size:38px;"><strong><%=porcentajeAvancePartida_lobby(session.Contents("cr"),rs3("componente"))%> %</strong></p></td>
         <td><p style="font-size:38px;color:green;text-shadow: 0 0 30px #315822;"><strong><span class="icon-star-full"></span></strong></p></td>
         </tr>
