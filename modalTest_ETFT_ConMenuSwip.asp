...
<table>
 <tr height="20px">
  <td><p><%if estatus1="1" then%>Obras por Proceso en Tiempo<%else%><%if estatus1="2" then%>Obras en Proceso con Atraso <%else%><%if estatus1="3" then%>Obras Terminadas <%end if%><%end if%><%end if%></strong></p></td>
 </tr>
</table>
...
<thead><tr><th width="2%">&nbsp;</th><th width="3%">CR</th><th width="10%">Suc.</th><th width="10%">Contrat.</th><th width="10%">Superv.</th><th width="5%">Fecha Inicio</th><th width="10%">Fecha Fin</th><th width="5%">Fecha Reprog.</th><th width="5%">% Avance</th><th width="5%">Estatus</th><%if cpsp="1" then%><th  width="5%">Pend.</th><%else%><th  width="5%">Costo Estim.</th><%end if%></tr></thead>
<tbody>
<%
...
if territorial="" then
 sql1= "SELECT a.cr, b.NOMBRE_SUCURSAL, c.contratista,e.login_nombre, b.INICIOPROYECTO,a.FechaTermino, d.EstatusObra, case when f.total is null then 0 else f.total end as total,a.fechareasignacion, round(a.avancereal,2) as avancereal FROM tseguimiento a LEFT JOIN cat_sucursales b ON a.cr = b.cr LEFT JOIN cat_contratistas c ON a.ContratistaAsignado = c.id_contratista LEFT JOIN cat_estatusobras d ON a.EstatusObra = d.id_estatusObra LEFT JOIN app_bnrt_login e on a.supervisorAsignado=e.login_id LEFT JOIN ( SELECT cr, ROUND(sum(cant * PRECIOBRUTO), 2) AS total FROM cat_conceptossucursales_sap GROUP BY cr) f ON a.cr*1 = f.cr *1 WHERE( a.EstatusObra = '"& estatus1 & "')  GROUP BY cr;"
else
 if  cpsp="1" then
  'sql1= "SELECT a.cr, b.NOMBRE_SUCURSAL, c.contratista,e.login_nombre, b.INICIOPROYECTO,a.FechaTermino, d.EstatusObra, case when f.total is null then 0 else f.total end as total FROM tseguimiento a LEFT JOIN cat_sucursales b ON a.cr = b.cr LEFT JOIN cat_contratistas c ON a.ContratistaAsignado = c.id_contratista LEFT JOIN cat_estatusobras d ON a.EstatusObra = d.id_estatusObra LEFT JOIN app_bnrt_login e on a.supervisorAsignado=e.login_id LEFT JOIN pendientes g on a.CR=g.cr LEFT JOIN ( SELECT cr, ROUND(sum(cant * PRECIOBRUTO), 2) AS total FROM cat_conceptossucursales_sap GROUP BY cr) f ON a.cr*1 = f.cr *1 WHERE( a.EstatusObra = '1' or a.EstatusObra = '2') and  b.territorial ='" & territorial &"' and g.id_estatus=1 GROUP BY cr;"
  if estatus1="3" then
   sql1="select * from (select a.cr,b.NOMBRE_SUCURSAL, b.Territorial,d.contratista,b.INICIOPROYECTO,b.FINPROYECTO as FechaTermino, case when c.pendientes is null then 0 else c.pendientes end as pendientes, f.login_nombre,G.estatusObra,a.fechareasignacion, round(a.avancereal,2) as avancereal from tseguimiento a LEFT JOIN cat_sucursales b on a.cr=b.CR LEFT JOIN (select cr,count(*) as pendientes from pendientes where id_estatus=1 GROUP BY cr) c on a.CR=c.cr LEFT JOIN cat_contratistas d on a.ContratistaAsignado=d.id_contratista LEFT JOIN cat_estatusobras g on a.EstatusObra=g.id_estatusObra LEFT JOIN app_bnrt_login f on a.supervisorAsignado=f.login_id where a.EstatusObra='"& estatus1 & "' and b.territorial='"& territorial &"')z where pendientes>0;"
  else
   sql1="select * from (select a.cr,b.NOMBRE_SUCURSAL, b.Territorial,d.contratista,b.INICIOPROYECTO,b.FINPROYECTO as FechaTermino, case when c.pendientes is null then 0 else c.pendientes end as pendientes, f.login_nombre,G.estatusObra,a.fechareasignacion, round(a.avancereal,2) as avancereal from tseguimiento a LEFT JOIN cat_sucursales b on a.cr=b.CR LEFT JOIN (select cr,count(*) as pendientes from pendientes where id_estatus=1 GROUP BY cr) c on a.CR=c.cr LEFT JOIN cat_contratistas d on a.ContratistaAsignado=d.id_contratista LEFT JOIN cat_estatusobras g on a.EstatusObra=g.id_estatusObra LEFT JOIN app_bnrt_login f on a.supervisorAsignado=f.login_id where (a.EstatusObra='1' or a.EstatusObra='2') and b.territorial='"& territorial &"')z where pendientes>0;"
  end if
 else
  sql1= "SELECT a.cr, b.NOMBRE_SUCURSAL, c.contratista,e.login_nombre, b.INICIOPROYECTO,a.FechaTermino, d.EstatusObra, case when f.total is null then 0 else f.total end as total,a.fechareasignacion, round(a.avancereal,2) as avancereal FROM tseguimiento a LEFT JOIN cat_sucursales b ON a.cr = b.cr LEFT JOIN cat_contratistas c ON a.ContratistaAsignado = c.id_contratista LEFT JOIN cat_estatusobras d ON a.EstatusObra = d.id_estatusObra LEFT JOIN app_bnrt_login e on a.supervisorAsignado=e.login_id LEFT JOIN ( SELECT cr, ROUND(sum(cant * PRECIOBRUTO), 2) AS total FROM cat_conceptossucursales_sap GROUP BY cr) f ON a.cr*1 = f.cr *1 WHERE( a.EstatusObra = '"& estatus1 & "') and  b.territorial = '"& territorial &"'  GROUP BY cr;"
  'sql1="select * from (select a.cr,b.NOMBRE_SUCURSAL, b.Territorial,d.contratista,b.INICIOPROYECTO,b.FINPROYECTO as FechaTermino, case when c.pendientes is null then 0 else c.pendientes end as pendientes, f.login_nombre,g.estatuss from tseguimiento a LEFT JOIN cat_sucursales b on a.cr=b.CR LEFT JOIN (select cr,count(*) as pendientes from pendientes where id_estatus=1 GROUP BY cr) c on a.CR=c.cr LEFT JOIN cat_contratistas d on a.ContratistaAsignado=d.id_contratista LEFT JOIN cat_estatusobras g on a.EstatusObra=g.id_estatusObra LEFT JOIN app_bnrt_login f on a.supervisorAsignado=f.login_id where a.EstatusObra='"& estatus1 & "' and b.territorial='"& territorial &"')z where pendientes>0;"
 end if
end if
rs1.open sql1, conn3
if rs1.eof then
...
if  cpsp="1" then
%>
<tr><td align="center" <%if estatus1<>0 then%> style="cursor:pointer"  onclick="showDetalleAvance('<%=rs1("cr")%>')"<%end if%>><font size="+2"><span class="icon-search" <%if estatus1 =0 then%>style="display:none"<%end if%>></span></font></td><td><font color="#000000"><%=rs1("cr")%></font></td><td><font color="#000000"><%=rs1("NOMBRE_SUCURSAL")%></font></td><td><font color="#000000"><%=rs1("contratista")%></font></td><td><font color="#000000"><%=rs1("login_nombre")%></font></td><td><font color="#000000"><%=rs1("INICIOPROYECTO")%></font></td><td><font color="#000000"><%=rs1("FechaTermino")%></font></td><td><font color="#000000"><%=rs1("fechareasignacion")%></font></td><td><font color="#000000"><%=rs1("avancereal")%></font></td><td><font color="#000000"><%=rs1("EstatusObra")%></font></td><td onclick="verPendientes('<%=rs1("cr")%>')"><%=rs1("pendientes")%></td></tr>
<%
else
%>
<tr><td align="center" <%if estatus1<>0 then%> style="cursor:pointer"  onclick="showDetalleAvance('<%=rs1("cr")%>')"<%end if%>><font size="+2"><span class="icon-search" <%if estatus1 =0 then%>style="display:none"<%end if%>></span></font></td><td><font color="#000000"><%=rs1("cr")%></font></td><td><font color="#000000"><%=rs1("NOMBRE_SUCURSAL")%></font></td><td><font color="#000000"><%=rs1("contratista")%></font></td><td><font color="#000000"><%=rs1("login_nombre")%></font></td><td><font color="#000000"><%=rs1("INICIOPROYECTO")%></font></td><td><font color="#000000"><%=rs1("FechaTermino")%></font></td><td><font color="#000000"><%=rs1("fechareasignacion")%></font></td><td><font color="#000000"><%=rs1("avancereal")%></font></td><td><font color="#000000"><%=rs1("EstatusObra")%></font></td><td><%=formatoPesos(nuevoCostoEstimado(rs1("cr")))%></td></tr>
<%
end if
rs1.movenext
	wend 
