...
function showOk(){
	simulateClick(document.getElementById('errOk'))
}
function showErrExiste(){
	simulateClick(document.getElementById('errExiste'))
}
</script>
...
<body onLoad="showErr()">
<%else%>
<%if request("err")="ok" then%>
<body onLoad="showOk()">
<%else%>
<%if request("err")="InsertErrorExistente" then%>
<body onLoad="showErrExiste()">
<%else%>
<body>
<%end if%>
<%end if%>
...
<table width='50%' align='center' cellspacing='0'>
	<h1 align='center'><font color='#991821'><strong>Asignacion de Gerentes</strong></font></h1>
</table>
	...
	<div id="asignarObra">
		<table width="80%" align="center">
 			<tr><td>
				<div class="datagrid">
					<table>
						<thead><tr><th>Gerente Asignado</th><th>Supervisor Asignado</th></tr></thead>
						<tbody>
							<tr><td><font color="#000000"><%=cargaGerentes()%></font></td><td><font color="#000000"><%=cargaSupervisores()%></font></td></tr>
						</tbody>
					</table>
                </div>    
			</td></tr>
...
<br/>
	...
	<a href="#" id="alert4" style="display:none"></a>
	  ...
	  <a href="#" id="errExiste" style="display:none">&nbsp;</a>
	  <a href="#" id="errOk" style="display:none">&nbsp;</a>
...
function validaFormulario(tabla){
			...
			//document.getElementById('form_edicion_usuarios').submit()
					document.getElementById('frm_revisarAvance').action="insertAsignarGerente.asp"
					document.getElementById("frm_revisarAvance").submit()
...
<script>
  ...
  $("#ok").on( 'click', function () {
		...
		$("#err").on( 'click', function () {
			reset();
			//$("#footer").html(document.getElementById('zonaFoto').value);
			alertify.error("No se pudo asignar el Gerente, vuelva a intentarlo");
			return false;
		});
		$("#errExiste").on( 'click', function () {
			reset();
			//$("#footer").html(document.getElementById('zonaFoto').value);
			alertify.error("No se pudo asignar el Gerente");
			alertify.error("Ya se ha realizado esta Operacion Antes");
			return false;
		});
		$("#errOk").on( 'click', function () {
			reset();
			//$("#footer").html(document.getElementById('zonaFoto').value);
			alertify.success("Gerente asignado con exito");
			return false;
		});
