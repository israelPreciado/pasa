...
function regresar(){
if(document.getElementById('pag').value=="atms"){
	document.getElementById('form_edicion_usuarios').action="EstimacionCostoObraATMS_lobby.asp"
	}
	else{
 document.getElementById('form_edicion_usuarios').action="EstimacionCostoObra_lobby.asp"
 }
...
function detSearchElemento(cajaTexto){
	$.ajax({
	type: "GET",
	url: "searchEstimacionObra_lobby.asp?cr=" + document.getElementById('cr').value + "&sap=" +document.getElementById('sap').value, 
	dataType: "html", 
	success: function(response){ 
...
function detCargaEdicion(cajaTexto){
	$.ajax({
	type: "GET",
	url: "carga_ElementosEstimacionCostoObra_lobby.asp?cr=" + document.getElementById('cr').value, 
	dataType: "html", 
	success: function(response){ 
...
function validarCampo(cajaTexto){	
	...
	else
	{
		if (/^[0-9\.]+$/.test(familia)){
			...
			{
				document.getElementById('cantidad').value=familia
...
function ingresarElemento(cajaTexto,zona,pu,componente){
 ...
 function detInserElemento(cajaTexto){
	$.ajax({
	type: "GET",
	url: "insertElementoEstimacionObra_lobby.asp?cr="+ document.getElementById('cr').value + "&cantidad=" + document.getElementById('unidad').value +"&sap=" + document.getElementById('sap').value + "&zona=" + document.getElementById('zona').value + "&pu=" + document.getElementById('pu').value + "&componente=" + document.getElementById('componente').value, 
	dataType: "html", 
	success: function(response){ 
	if (response=="ok"){
...
function editarElemento(cajaTexto){
document.getElementById('parrafo_'+cajaTexto).style.display="none"
document.getElementById(cajaTexto).style.display="block"
document.getElementById('editar_'+cajaTexto).style.display="none"
document.getElementById('save_'+cajaTexto).style.display="block"
}
function editarElemento2(cajaTexto){
	simulateClick(document.getElementById('errorEU'))
}

function guardarElemento(cajaTexto){
if(validarCampo(cajaTexto)){
document.getElementById('form_edicion_usuarios').action="updateElementoEstimacionObra_lobby.asp"
document.getElementById('form_edicion_usuarios').submit()
}
}

function eliminarElemento(cajaTexto){

document.getElementById('sap').value=cajaTexto
document.getElementById('form_edicion_usuarios').action="deleteElementoEstimacionObra_lobby.asp"
document.getElementById('form_edicion_usuarios').submit()
}
...
<tr height="20">
 ...
 <tr>
  <td><font size="+2"><strong>Bienvenido Estrategia Integral</strong></font></td>
 </tr> 
 <tr height="20">
...
<table cellpadding="0" cellspacing="0" class="display" id="example1" width="100%">
 <thead>
  <tr bgcolor="#991821">
    ...
    <th width="40%"  height="42" align="center"><font color="#FFFFFF" size="-1">DESCRIPCION LARGA</font></th>
	...
	<th width='3%'  height='42' align='center'><font color='#FFFFFF' size='-1'>ESTIMACION</font></th>
	...
	<th width="6%"  height="42" align="center"><font color="#FFFFFF" size="-1">&nbsp;</font></th>
  </tr>
 </thead> 
 
  <%=cargaClavesSapCargadas_lobby(cr)%>
...
<script>
  ...
  $("#ok").on( 'click', function () {
		...
		$("#errorEU").on( 'click', function () {
			reset();
			//$("#footer").html(document.getElementById('zonaFoto').value);
			alertify.error("Este elemento ya se cerro");
			return false;
		});
