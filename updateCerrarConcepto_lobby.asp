...
set rs2=server.CreateObject("adodb.recordset")
sql5="select * from nuevoCosto_estimadoObra_lobby where id='" & sap & "' and cr='" & cr & "'"
rs2.open sql5, conn
...
'********** hacemos un BEGIN para poder realizar posteriormente un commit o un rollback
	...
	sql="update nuevoCosto_estimadoObra_lobby set avanceCompletado='1' where id='" & sap & "' and cr='" & session("cr") & "'"
	...
	if err<>0 then
		...
		sql2="insert into nuevoCosto_estimadoObra__lobby_log (cr,componente,clave_sap_concurso) values ('" & cr & "','UPDATE/Cierra Concepto','" & sap & "')"
		'ejecutamos la consulta
		conn.execute sql2
	...
	if porcentaje>max then
		...
		'comento esto ya que no se puede el exceso aun++++++++++++++++++++++++++++++++
	...
	'		sql12= "select * from tseguimiento_lobby where cr='" & cr & "'"
	...
	'					sql13="update tseguimiento_lobby set EstatusObra=2 , last_update ='"  &  Year(Now) & "-" & LPad(Month(Now),2,"0")  & "-" & LPad(Day(Now),2,"0") & "' where cr='" & rs12("cr") & "'"
	'					conn.execute sql13
	'					sql13="update tseguimiento_lobby set avanceProyectado=" & cDbl(calculaAvanceProyectado7030(rs12("cr"),rs12("fechaInicio"))) & " where cr='" & rs12("cr") & "'"
	'					conn.execute sql13
	'				else
	'					if (cDbl(calculaAvanceProyectado7030(rs12("cr"),rs12("fechaInicio")))<cDbl(calculaAvanceGeneral(rs12("cr"),"G"))) then
