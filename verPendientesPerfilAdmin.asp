...
function verImagenes(idControl){
	...
	document.getElementById('frm_avanceObra').action="verImagenesPendientesAdmin.asp"
  	...
  	<div id="Obra_civil">
  		<div align="center" id="menu_inicio">
    		<div id="detalle">
    			<table width="100%">
     				<tr>
     				 	<td width="70%"><p>CR: <strong><%=cr%></strong></p></td>
		...
		<br />
  		<div id="logo">  
    		<p><strong>Relaci&oacute;n de Pendientes</strong></p>
...
while not (rs3.eof)
'sql3= "SELECT a.*, CASE WHEN b.abiertos IS NULL THEN 0 ELSE b.abiertos END AS abiertos, CASE WHEN c.cerrados IS NULL THEN 0 ELSE c.cerrados END AS cerrados FROM pendientes a LEFT JOIN( SELECT cr AS cr, count(*) AS abiertos FROM pendientes WHERE cr = '" & session.Contents("cr") & "' AND id_modulo ='" & rs3("id_modulo") & "' AND id_estatus = 1) b ON a.cr = b.cr LEFT JOIN( SELECT cr, count(*) AS cerrados FROM pendientes WHERE cr = '" & session.Contents("cr") & "' AND id_modulo = '" & rs3("id_modulo") & "' AND id_estatus = 0) c ON a.cr = c.cr WHERE a.cr = '" & session.Contents("cr") & "' AND a.id_modulo = '" & rs3("id_modulo") & "';"
sql3= "SELECT a.*, f.perfil,DATE_FORMAT(fecha_reporte, '%Y-%m-%d %h:%i:%s') as fecha_reporte2,DATE_ADD(left(fecha_reporte,10) ,INTERVAL  7 DAY) AS FECHA_VENCIMIENTO, CASE WHEN b.abiertos IS NULL THEN 0 ELSE b.abiertos END AS abiertos, CASE WHEN c.cerrados IS NULL THEN 0 ELSE c.cerrados END AS cerrados FROM pendientes a left join cat_perfil f on a.perfilregistradoPor =f.ID_PERFIL LEFT JOIN( SELECT cr AS cr, count(*) AS abiertos FROM pendientes WHERE cr = '" & cr & "' AND id_modulo ='" & rs3("id_modulo") & "' AND id_estatus = 1) b ON a.cr = b.cr LEFT JOIN( SELECT cr, count(*) AS cerrados FROM pendientes WHERE cr = '" & cr & "' AND id_modulo = '" & rs3("id_modulo") & "' AND id_estatus = 0) c ON a.cr = c.cr WHERE a.cr = '" & cr & "' AND a.id_modulo = '" & rs3("id_modulo") & "';"
	rs6.open sql3, conn3
	if rs6.eof then
		rs6.close
		%>
	...
	%>
        <tr>
 		...
 		<tr>
         <td colspan="3">
          <table style="display:none" id="tabla<%=rs3("id_modulo")%>" width="100%">
         <%
		while not(rs6.eof)	
		%>
		
			<tr>    
      			<td width="100%" valign="middle" align="left" colspan="3"><font size="+1"><p><strong>Registrado por:</strong><%=rs6("perfil")%></font></p></td>	
            </tr>
			<tr>    
      			<td width="100%" valign="middle" align="left" colspan="3"><font size="+1"><p><strong>Fecha de Registro:</strong><%=rs6("fecha_reporte2")%></font></p></td>	
            </tr>
			<tr>    
      			<td width="100%" valign="middle" align="left" colspan="3"><font size="+1"><p><strong>Fecha de Vencimiento:</strong><%=rs6("FECHA_VENCIMIENTO")%></font></p></td>	
            </tr>
   			<tr>    
			...
			<tr>    
      			<td width="100%" valign="middle" align="left" colspan="3"><font size="+1"><p id="pInstruccion<%=rs6("id_pendiente")%>" style="display: none;"><strong>Puedes Ingresar un Comentario</strong></font></p></td>	
            </tr>
			<tr>    
      			<td width="100%" valign="middle" align="left" colspan="3"><textarea class="area" id="input_comentario<%=rs6("id_pendiente")%>" name="input_comentario<%=rs6("id_pendiente")%>" type="text" style="display: none;" placeholder="comentario">&nbsp;</textarea></td>	
            </tr>
			<tr>    
      			<td width="100%" valign="middle" align="left" colspan="3">&nbsp;</td>	
            </tr>
