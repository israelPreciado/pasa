...
<thead><tr><th>&nbsp;</th><th>CR</th><th>Suc.</th><th>Contrat.</th><th>Superv.</th><th>Fecha Inicio</th><th>Fecha Fin</th><th>Fecha Reprog.</th><th>Estatus</th><th>Pend.</th><th>% Avance</th></tr></thead>
<tbody>
<%
...
'sql1= "SELECT a.cr, b.NOMBRE_SUCURSAL, c.contratista,e.login_nombre, b.INICIOPROYECTO, d.EstatusObra, f.total FROM tseguimiento a LEFT JOIN cat_sucursales b ON a.cr = b.cr LEFT JOIN cat_contratistas c ON a.ContratistaAsignado = c.id_contratista LEFT JOIN cat_estatusobras d ON a.EstatusObra = d.id_estatusObra LEFT JOIN app_bnrt_login e on a.supervisorAsignado=e.login_id LEFT JOIN (select cr, round(sum(monto),2) as total from costoestimado GROUP BY cr) f on a.cr=f.cr WHERE( a.EstatusObra = '"& estatus1 & "' OR a.EstatusObra = '"& estatus2 & "') AND b.territorial = '"& territorial &"';"
  ...
  sql1= "select a.cr,b.NOMBRE_SUCURSAL,d.contratista,e.login_nombre, c.FechaInicio,c.FechaTermino,c.FechaReasignacion,f.estatusObra,g.pendientes, c.avancereal from pendientes a LEFT JOIN cat_sucursales b on a.cr=b.CR LEFT JOIN tseguimiento c on a.cr=c.cr LEFT JOIN cat_contratistas d on c.ContratistaAsignado=d.id_contratista LEFT JOIN app_bnrt_login e on c.supervisorasignado=e.login_id LEFT JOIN cat_estatusobras f on c.EstatusObra=f.id_estatusObra left join (select cr,count(*) as pendientes from pendientes where id_estatus=1 GROUP BY cr) g on a.cr=g.cr where a.id_estatus=1 AND DATEDIFF(now(),a.fecha_reporte) > 3  GROUP BY a.CR"
...
'sql1= "SELECT a.cr, b.NOMBRE_SUCURSAL, c.contratista,e.login_nombre, b.INICIOPROYECTO, d.EstatusObra, f.total FROM tseguimiento a LEFT JOIN cat_sucursales b ON a.cr = b.cr LEFT JOIN cat_contratistas c ON a.ContratistaAsignado = c.id_contratista LEFT JOIN cat_estatusobras d ON a.EstatusObra = d.id_estatusObra LEFT JOIN app_bnrt_login e on a.supervisorAsignado=e.login_id LEFT JOIN (select cr, round(sum(monto),2) as total from costoestimado GROUP BY cr) f on a.cr=f.cr WHERE( a.EstatusObra = '"& estatus1 & "' OR a.EstatusObra = '"& estatus2 & "') AND b.territorial = '"& territorial &"';"
  ...
  sql1= "select a.cr,b.NOMBRE_SUCURSAL,d.contratista,e.login_nombre, c.FechaInicio,c.FechaTermino,c.FechaReasignacion,f.estatusObra,g.pendientes, c.avancereal from pendientes a LEFT JOIN cat_sucursales b on a.cr=b.CR LEFT JOIN tseguimiento c on a.cr=c.cr LEFT JOIN cat_contratistas d on c.ContratistaAsignado=d.id_contratista LEFT JOIN app_bnrt_login e on c.supervisorasignado=e.login_id LEFT JOIN cat_estatusobras f on c.EstatusObra=f.id_estatusObra left join (select cr,count(*) as pendientes from pendientes where id_estatus=1 GROUP BY cr) g on a.cr=g.cr where a.id_estatus=1 AND DATEDIFF(now(),a.fecha_reporte) > 3 and b.zona_territorial='" & session.contents("zonaDirector") & "' GROUP BY a.CR"
  'sql1="select * from (select a.cr,b.NOMBRE_SUCURSAL, b.Territorial,d.contratista,b.INICIOPROYECTO,b.FINPROYECTO as FechaTermino, case when c.pendientes is null then 0 else c.pendientes end as pendientes, f.login_nombre,g.estatuss from tseguimiento a LEFT JOIN cat_sucursales b on a.cr=b.CR LEFT JOIN (select cr,count(*) as pendientes from pendientes where id_estatus=1 GROUP BY cr) c on a.CR=c.cr LEFT JOIN cat_contratistas d on a.ContratistaAsignado=d.id_contratista LEFT JOIN cat_estatusobras g on a.EstatusObra=g.id_estatusObra LEFT JOIN app_bnrt_login f on a.supervisorAsignado=f.login_id where a.EstatusObra='"& estatus1 & "' and b.territorial='"& territorial &"')z where pendientes>0;"
