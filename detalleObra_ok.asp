...
<table width="100%" align="center">
 ...
 <tr>
  <td align="center" width="25%"><p><font size="+3"><strong>CR</strong></font></p></td><td align="center" width="30%"><p><font size="+3"><strong>TERR.</strong></font></p></td><td align="center" width="20%"><p><font size="+3"><strong>FET</strong></font></p></td><td align="center" width="25%"><p><font size="+3"><strong>Pendientes</strong></font></p></td>
 </tr>
 <%else%>
  <tr>
  <td width="10%">&nbsp;</td><td align="center" width="25%"><p><font size="+3"><strong>CR</strong></font></p></td><td align="center" width="40%"><p><font size="+3"><strong>SUC.</strong></font></p></td><td align="center" width="25%"><p><font size="+3"><strong>Pendientes</strong></font></p></td>
 </tr>
 <%end if%>
 <%
...
sql2= "select * from (SELECT a.CR,a.sucursal as nombre_sucursal, a.FechaTermino, CASE WHEN b.pendientes IS NULL THEN 0 ELSE b.pendientes END AS pendientes FROM tseguimiento a LEFT JOIN( SELECT cr, COUNT(cr) AS pendientes FROM pendientes where id_estatus=1 GROUP BY cr) b ON a.cr = b.cr WHERE (a.EstatusObra = '" & estatusObra & "' or a.EstatusObra='" & estatusObra2 & "')  ORDER BY FechaTermino asc)a where pendientes>0"
end if
...
sql= "SELECT a.CR,c.territorial,c.nombre_sucursal, a.FechaTermino, CASE WHEN b.pendientes IS NULL THEN 0 ELSE b.pendientes END AS pendientes FROM tseguimiento a LEFT JOIN app_bnrt_login d on a.supervisorAsignado =d.login_id LEFT JOIN cat_sucursales c on a.CR=c.CR LEFT JOIN( SELECT cr, COUNT(cr) AS pendientes FROM pendientes where id_estatus=1 GROUP BY cr) b ON a.cr = b.cr WHERE (a.EstatusObra = '" & estatusObra & "' or a.EstatusObra='" & estatusObra2 & "')  and c.territorial='" & division &"' and d.login_nombre like '"& supervisor &"%' ORDER BY FechaTermino asc"
sql2= "select * from (SELECT a.CR,a.sucursal as nombre_sucursal, a.FechaTermino, CASE WHEN b.pendientes IS NULL THEN 0 ELSE b.pendientes END AS pendientes FROM tseguimiento a LEFT JOIN( SELECT cr, COUNT(cr) AS pendientes FROM pendientes where id_estatus=1 GROUP BY cr) b ON a.cr = b.cr WHERE (a.EstatusObra = '" & estatusObra & "' or a.EstatusObra='" & estatusObra2 & "')  ORDER BY FechaTermino asc)a where pendientes>0"
end if
...
'otro perfil
sql= "SELECT a.CR, a.FechaTermino,c.nombre_sucursal, CASE WHEN b.pendientes IS NULL THEN 0 ELSE b.pendientes END AS pendientes FROM tseguimiento a LEFT JOIN cat_sucursales c on a.CR=c.CR LEFT JOIN( SELECT cr, COUNT(cr) AS pendientes FROM pendientes where id_estatus=1 GROUP BY cr) b ON a.cr = b.cr WHERE (a.EstatusObra = '" & estatusObra & "' or a.EstatusObra='" & estatusObra2 & "') and a.ContratistaAsignado='" & session.Contents("contratista") & "' ORDER BY FechaTermino asc"
...
if not(isempty(session.Contents("director"))) then
%>
<tr height="80px"><td style="padding-left:10%" onclick="enviar('<%=rs("cr")%>')"><font size="100%" color='#d51818' style="text-decoration:underline"><strong><%=rs("cr")%></strong></font></td><td><font size="+3"><%=rs("territorial")%></font></td><td><font size="+3"><%=rs("FechaTermino")%></font></td><td style="padding-left:10%"><font size="100%"><%=rs("pendientes")%></font></td></tr>
<%
else
%>
<tr height="80px"><td align="left"  onclick="detSucursal('<%=rs("cr")%>')"><a href="#"><font style="font-size:200%;"><span class="icon-eye"></span></font></a></td><td style="padding-left:10%" onclick="enviar('<%=rs("cr")%>')"><font size="100%" color='#d51818' style="text-decoration:underline"><strong><%=rs("cr")%></strong></font></td><td align="center"><font size="+3"><%=rs("nombre_sucursal")%></font></td><td style="padding-left:10%"><font size="100%"><%=rs("pendientes")%></font></td></tr>
<% 
end if
rs.movenext
wend
