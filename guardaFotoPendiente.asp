	...
	if sinFoto="sinFoto" and isempty(objFich) then
		...
		if objFich.TipoContenido="image/jpeg" or  objFich.TipoContenido="image/png"  or objFich.TipoContenido="image/bmp" then 
			if tamano >= 6000000 then
				...
				if cerrar="cerrar" then
					sql="insert into ctrl_fotografiasPendientes (cr,id_modulo,name,path,id_privilegio,id_usuario,id_pendiente,comentario) values ('" & cr & "'," & modulo & ",'" & nombreArchivo & "','adjuntos/Pendientes/" & modulo & "/'," & session.Contents("privilegio") & ",'" & session.Contents("nombre_usuario") & "'," & id_pendiente & ",'" & comentario & "')"
				else
