...
sql="insert into ctrl_fotografias (cr,id_zona,name,path,comentario,usuario,perfil) values ('" & cr & "'," & zona & ",'" & nombreArchivo & "','adjuntos/" & nombreZona & "/','" & comentario & "','" & usuario & "'," & cint(perfil) & ")"
...
if err<>0 then
	conn.close 
	set conn=nothing
 	response.redirect("modulos.asp?res=err")
else
...
'conn.execute sql2
	conn.close 
	set conn=nothing
	response.redirect("modulos.asp?res=ok")
end if
