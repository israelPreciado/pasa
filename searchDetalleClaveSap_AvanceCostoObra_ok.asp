...
Conn5.Open connstr
sql= "SELECT *, ROUND((CANTIDAD * PU) *(PORAVANCE / 100), 2) AS porAvancePesos FROM( SELECT * FROM( SELECT a.id, a.CLAVE_SAP_CONCURSO, a.clave_sap_zona, b.DESCRIPCION_CORTA, b.descripcion_larga, a.pu, a.cantidad, b.UND, a.componente, concat( LEFT(a.fecha_ingreso, 4), '-', RIGHT(LEFT(a.fecha_ingreso, 7), 2), '-', RIGHT(LEFT(a.fecha_ingreso, 10), 2), ' ', RIGHT(a.fecha_ingreso, 8)) AS fecha_ingreso, a.avance, a.avanceContratista, CASE WHEN a.avance >= 0 THEN round((a.avance * 100) / a.cantidad, 2) ELSE 0 END AS porAvance FROM nuevoCosto_estimadoObra a LEFT JOIN cat_sap_2017 b ON a.CLAVE_SAP_CONCURSO = b.CLAVE_SAP_CONCURSO WHERE cr = '" & session.contents("cr") & "' AND a.componente NOT LIKE 'Otros' AND a.componente NOT LIKE 'atms') AS a UNION SELECT * FROM( SELECT a.id, a.CLAVE_SAP_CONCURSO, a.clave_sap_zona, b.DESCRIPCION_CORTA, b.descripcion_larga, a.pu, a.cantidad, b.UND, a.componente, concat( LEFT(a.fecha_ingreso, 4), '-', RIGHT(LEFT(a.fecha_ingreso, 7), 2), '-', RIGHT(LEFT(a.fecha_ingreso, 10), 2), ' ', RIGHT(a.fecha_ingreso, 8)) AS fecha_ingreso, a.avance, a.avanceContratista, CASE WHEN a.avance >= 0 THEN round((a.avance * 100) / a.cantidad, 2) ELSE 0 END AS porAvance FROM nuevoCosto_estimadoObra a LEFT JOIN cat_sap_atms_2017 b ON a.CLAVE_SAP_CONCURSO = b.CLAVE_SAP_CONCURSO WHERE cr = '" & session.contents("cr") & "' AND a.componente NOT LIKE 'Otros' AND a.componente LIKE 'atms') AS b UNION( SELECT id, clave_sap_concurso, clave_sap_zona, descorta AS descripcion_corta, deslarga AS descripcion_larga, pu, cantidad, und, componente, fecha_ingreso, avance, avanceContratista, CASE WHEN avance >= 0 THEN(avance * 100) / cantidad ELSE 0 END AS porAvance FROM nuevocosto_estimadoobra WHERE cr = '" & session.contents("cr") & "' AND componente LIKE 'Otros') ORDER BY fecha_ingreso DESC) a WHERE id ='" & id & "'"
rs25.open sql,conn5
if rs25.eof then
...
while not(rs25.eof)
 ...
 regresa=regresa & "<h2 align='center'>CLAVE SAP " & rs25("clave_sap_concurso") & "</h2>"
       ...
       regresa=regresa & "<div class='w3-container'>"
		 ...
		 regresa=regresa & "<p><strong>CANTIDAD ESTIMADA: </strong> " &  rs25("CANTIDAD") &" " & rs25("UND") &"</p>"
