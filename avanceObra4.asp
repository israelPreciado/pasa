...
%> 
    ...
    <table width="100%">
     ...
     <tr>
      <td width="50%"><p>&nbsp;</p></td>
	  <td><p><strong>&nbsp;</strong></p></td>
      <td width="20%"><p><strong>Sup</strong></p></td>
	  <td><p><strong>&nbsp;</strong></p></td>
...
<%else%>
 <%if modulo="1" then%>
	<tr>
      <td width="55%"><p><%=rs("componente")%></p></td>
      <td width="10%"><img src="images/logos/loading.gif"  id="OB_load<%=rs("id_componente")%>" style="display:none"/><img src="images/logos/update.png" id="OB_update<%=rs("id_componente")%>" onclick="showGif(this.id,'OB_load<%=rs("id_componente")%>','parrafo<%=rs("id_componente")%>','select<%=rs("id_componente")%>');fieldUpdate('<%=rs("nameBD")%>','<%=session.Contents("cr")%>',<%=rs("id_componente")%>)" style="display:none"/><%if session.Contents("privilegio")=2 then%><img src="images/logos/edit.png" id="OB_edit<%=rs("id_componente")%>" onclick="showGif(this.id,'OB_update<%=rs("id_componente")%>','select<%=rs("id_componente")%>','parrafo<%=rs("id_componente")%>')"/><%end if%></td>
      <td><%=cargaPorcentajesAvance(rs("id_componente"))%> <p id="parrafo<%=rs("id_componente")%>"><%=cargaAvance(rs("nameBD"),session.Contents("cr"))%></p></td><td align="right"><%=cargaPorcentajesAvance(rs("id_componente"))%> <p id="parrafo<%=rs("id_componente")%>"><%=cargaAvanceContratista(rs("nameBD"),session.Contents("cr"))%></p></td>    
     </tr>
 <%else%>
	<tr>
      <td width="55%"><p><%=rs("componente")%></p></td>
      <td width="10%"><img src="images/logos/loading.gif"  id="OB_load<%=rs("id_componente")%>" style="display:none"/><img src="images/logos/update.png" id="OB_update<%=rs("id_componente")%>" onclick="showGif(this.id,'OB_load<%=rs("id_componente")%>','parrafo<%=rs("id_componente")%>','select<%=rs("id_componente")%>');fieldUpdate('<%=rs("nameBD")%>','<%=session.Contents("cr")%>',<%=rs("id_componente")%>)" style="display:none"/><%if session.Contents("privilegio")=2 then%><img src="images/logos/edit.png" id="OB_edit<%=rs("id_componente")%>" onclick="showGif(this.id,'OB_update<%=rs("id_componente")%>','select<%=rs("id_componente")%>','parrafo<%=rs("id_componente")%>')"/><%end if%></td>
      <td><%=cargaPorcentajesAvanceModulos(rs("id_componente"))%> <p id="parrafo<%=rs("id_componente")%>"><%=cargaAvance(rs("nameBD"),session.Contents("cr"))%></p></td><td align="right"><%=cargaPorcentajesAvanceModulos(rs("id_componente"))%> <p id="parrafo<%=rs("id_componente")%>"><%=cargaAvanceContratista(rs("nameBD"),session.Contents("cr"))%></p></td>    
     </tr>
 <%end if%>	 
<%end if%>
<%
