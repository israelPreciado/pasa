...
'seleccionamos la consulta segun el parametro de entrada
    ...
    sql="select a.cr,a.Sucursal,b.Territorial,a.TipoTransformacion,a.FechaInicio,a.FechaTermino,c.login_nombre,d.contratista,a.estatusObra,a.comentarioEstatus from tseguimiento a LEFT JOIN cat_sucursales b on a.cr=b.cr LEFT JOIN app_bnrt_login c on a.supervisorasignado=c.login_id LEFT JOIN cat_contratistas d on a.ContratistaAsignado=d.id_contratista where a.cr='" & cr & "'"
...
regresar=" <div id='myModal' class='w3-modal'>"
regresar=regresar & "<div class='w3-modal-content w3-card-4'>"
regresar=regresar & "      <header class='w3-container w3-red'> "
...
'dependiendo del parametro de entrada se le llama una funcion que te redirige a la pagina de detalle	
	...
	regresar=regresar & " <p style='color:blue'><strong>Ingrese el Pedimento de Obra </strong></p>"
	regresar=regresar & "<input type='text' id='txt_pedimento'  onkeypress='return soloNumeros(event)'><font color='red' size='34'><span class='icon-floppy-disk' style='cursor:pointer' onclick='updatePedimento(txt_pedimento.value)' </span></font></p>"
	if rs("estatusObra")="4" then
	regresar=regresar & " <p><strong>Observaciones: </strong>" & rs("comentarioEstatus") &"</p>"
	end if
rs.movenext
wend
regresar=regresar & "      </div>"
