...
function verDetalle(regional){
document.getElementById('regional').value=regional
document.getElementById('frm_revisarAvance').action="moduloFinanciero_ConMenuSwip_paso2.asp"
document.getElementById('frm_revisarAvance').submit()
}
function cierraDiv(){
...
sql1="SELECT *, round( 100 - ifnull((( ifnull(presupuesto, 0) - ifnull(costoReal, 0)) * 100 / presupuesto), 0), 2) AS Desviacion FROM( SELECT a.regional, a.territorial, round(ifnull(b.total, 0), 2) AS estimacion1, round(ifnull(c.total, 0), 2) AS estimacion2, round(ifnull(d.total, 0), 2) AS estimacion3, round( ifnull(b.total, 0) + ifnull(c.total, 0) + ifnull(d.total, 0), 2) AS costoReal, round(ifnull(e.total, 0), 2) AS presupuesto FROM( SELECT regional, Territorial FROM cat_sucursales WHERE zona_territorial = '" & zona_territorial & "' AND territorial = '" & territorial & "' GROUP BY regional) a LEFT JOIN( SELECT sum(totalestimacion) AS total, regional, territorial FROM( SELECT a.*, pu * avance AS totalEstimacion, b.regional, b.territorial, concat( ifnull(a.cr, '0000'), ifnull( clave_sap_concurso, '000000000'), ifnull(clave_sap_zona, '00000000'), cantidad, pu) AS nid FROM estimacion1 a LEFT JOIN cat_sucursales b ON a.cr = b.cr WHERE b.zona_territorial = '" & zona_territorial & "' AND b.territorial = '" & territorial & "' GROUP BY concat( ifnull(a.cr, '0000'), ifnull( clave_sap_concurso, '000000000'), ifnull(clave_sap_zona, '00000000'), cantidad, pu)) a GROUP BY regional) b ON a.regional = b.regional LEFT JOIN( SELECT sum(totalestimacion) AS total, regional FROM( SELECT a.*, pu * avance AS totalEstimacion, b.regional, b.territorial, concat( ifnull(a.cr, '0000'), ifnull( clave_sap_concurso, '000000000'), ifnull(clave_sap_zona, '00000000'), cantidad, pu) AS nid FROM estimacion2 a LEFT JOIN cat_sucursales b ON a.cr = b.cr WHERE b.zona_territorial = '" & zona_territorial & "' AND b.territorial = '" & territorial & "' GROUP BY concat( ifnull(a.cr, '0000'), ifnull( clave_sap_concurso, '000000000'), ifnull(clave_sap_zona, '00000000'), cantidad, pu)) a GROUP BY regional) c ON a.regional = c.regional LEFT JOIN( SELECT sum(totalestimacion) AS total, regional FROM( SELECT a.*, pu * avance AS totalEstimacion, b.regional, b.Territorial, concat( ifnull(a.cr, '0000'), ifnull( clave_sap_concurso, '000000000'), ifnull(clave_sap_zona, '00000000'), cantidad, pu) AS nid FROM estimacion3 a LEFT JOIN cat_sucursales b ON a.cr = b.cr WHERE b.zona_territorial = '" & zona_territorial & "' AND b.territorial = '" & territorial & "' GROUP BY concat( ifnull(a.cr, '0000'), ifnull( clave_sap_concurso, '000000000'), ifnull(clave_sap_zona, '00000000'), cantidad, pu)) a GROUP BY regional) d ON a.regional = d.regional LEFT JOIN( SELECT sum(total) AS total, regional FROM( SELECT sum(cantidad * pu) total, regional FROM( SELECT a.*, concat( ifnull(a.cr, '0000'), ifnull( clave_sap_concurso, '000000000'), ifnull(clave_sap_zona, '00000000'), cantidad, pu, CASE WHEN componente = 'otros' THEN descorta ELSE '' END) AS nid, b.Regional, b.Territorial FROM nuevocosto_estimadoobra a LEFT JOIN cat_sucursales b ON a.cr = b.cr WHERE b.zona_territorial = '" & zona_territorial & "' AND b.territorial = '" & territorial & "') a GROUP BY nid DESC) a GROUP BY regional) e ON a.regional = e.regional) a WHERE presupuesto > 0"
...
%>
 <tr>
  <td align="center" onclick="verDetalle('<%=rs1("regional")%>')"><font size="+2" color="red"><span class="icon-search"></span></font></td>
  <td><font color="#000000"><%=rs1("regional")%></font></td>
